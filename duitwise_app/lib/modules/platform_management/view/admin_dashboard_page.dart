...
/// 1) Stream provider to show existing lessons
/// Change `watchLessons()` to your actual repository method name if needed.
final adminLessonsProvider = StreamProvider.autoDispose<List<Lesson>>((ref) {
  final repo = ref.watch(lessonRepositoryProvider);
  return repo.watchLessons(); // <- rename if your repo uses different name
});

/// 2) Controller for admin actions (Riverpod 3.0)
final lessonControllerProvider =
    AsyncNotifierProvider<LessonController, void>(LessonController.new);
  ...
  void _addOutcome() {
    ...
    if (_outcomes.contains(text)) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Outcome already added')),
      );
      return;
    }
                                  ...
                                  Expanded(
                                    child: Column(
                                      ...
                                      children: [
                                        ...
                                        Text(
                                          '${lesson.category} • ${lesson.difficulty == 1 ? 'Beginner' : lesson.difficulty == 2 ? 'Intermediate' : 'Advanced'}',
                                        ),
                                      ],
                                    ),
                                  ),
